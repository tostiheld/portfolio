using System;
using System.Linq;

using Newtonsoft.Json;
using Newtonsoft.Json.Linq;
using LinqToDB;

using Roadplus.Server.API;
using Roadplus.Server.Data;

namespace Roadplus.Server.Messages.Json
{
    public class RemoveSchoolCommand : ICommand
    {
        public string Name { get { return "removeSchool"; } }

        private const string idKey = "schoolId";

        public IResponse Execute(string payload)
        {
            JObject o = JsonConvert.DeserializeObject<JObject>(payload);

            int id;

            if (!Int32.TryParse(o[idKey].ToString(), out id))
            {
                return new ResponseFailure(Name, "Parse error");
            }

            RoadplusData data = new RoadplusData();

            data.Schools.Where(s => s.SchoolId == id).Delete();

            return new RemoveResponse()
            {
                Command = Name,
                RemovedObjectId = id
            };
        }

    }
}

