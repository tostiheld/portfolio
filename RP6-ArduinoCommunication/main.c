 ...
 */ 
 ...
 //gets the speed limit from the arduino
 int getSpeedLimit(int binarySpeed)
 {
	 speed = binarySpeed;
	 writeInteger(speed, DEC);
	 return speed;
 }
 //handles rc5 data from the tv remote
...
//#ifndef DO_NOT_MOVE    	// used to disable movement if you want to 
 ...
 // used to store if we have received 
	writeInteger(speed, DEC);								  // a movement command.
	if (speed == 0)
	{
		
			speed = 30;
	}	
	writeInteger(speed, DEC);							  // Any other key is ignored!
...
void task_updateRegisters(void)
{
	if(!I2CTWI_readBusy) 
	{
		//I2CTWI_readRegisters[I2C_REG_SPEED] = 		 (uint8_t)(getSpeedLimit());
...
uint8_t getCommand(void)
{
	if(I2CTWI_writeRegisters[I2C_REGW_CMD] && !I2CTWI_writeBusy) 
	{
		...
		param7 = I2CTWI_writeRegisters[I2C_REGW_CMD_SPEED];
		return true;
	}
	return false;
}
...
void task_commandProcessor(void)
{
	if(getCommand()) 
	{
		switch(cmd) 
		{
			...
			case CMD_SET_SPEED: getSpeedLimit((int)param7); break;
				switch(param1)
				{
					case ACS_PWR_OFF: 
